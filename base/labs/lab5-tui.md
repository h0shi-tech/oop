...
**⚠ Данный документ не содержит все сведения, необходимые для выполнения лабораторной работы.
Отсутствующую информацию см. в документации:**
...
При этом цикл, запрос событий, обработку событий и отрисовку (то есть *весь код*) пишет сам программист
(используя при необходимости функции и типы, предоставляемые библиотекой).

При использовании *фреймворка* у вас отбирается контроль над основным потоком выполнения.
Задача разработчика заключается в том, чтобы предоставить некоторые куски кода, которые будут вызываться фреймворком.

Резюмируя:
- при использовании библиотеки программист вызывает библиотечный код;
- при использовании фреймворка фреймворк вызывает код программиста.
...
что при использовании фреймворка программисту не требуется самому писать цикл обработки событий и отрисовки.
Реализация такого цикла с получением событий, обработкой событий и отрисовкой кадра спрятано внутри фреймворка.
...
- ⚠ Работа с фреймворком в основном подразумеват использование обычных указателей.
...
- **Вызов конструктора одного из базовых классов `TProgInit`.**
  ...
  В данном примере:
  - для создания строки статуса используется статический метод `TApp::initStatusLine`;
  - меню не создаётся;
  - для создания рабочего стола используется статический метод `TApp::initDeskTop`.
...
- **Пользовательский метод `initStatusLine`.**
  ...
  Полный набор отображаемых команд определяется текущим **контекстом подсказки** (это обычное число, изначально равное 0).
  ...
  Если текущий контекст подсказки попадает в диапазон [`min`, `max`],
  то все подсказки из данного контейнера `TStatusDef` будут отображаться в строке статуса.
  ...
  - Контейнер `TStatusDef`, определяющий наборы отображаемых команд.
    ...
    1. У одной перегрузки правым операндом является `TStatusItem` (добавляет новый элемент в контейнер).
    2. У другой перегрузки правым операндом является `TStatusDef` (добавляет новый контейнер).

- **Пользовательский метод `initDeskTop`.**

  Конструктор `TDeskTop` принимает прямоугольник, задающий место расположения рабочего стола.
  В примере выше область рабочего стола - это всё окно, за исключеним строки статуса.

- **Задача для самостоятельного закрепления материала.**

  Модифицируйте вышеприведённый код таким образом, чтобы строка состояния оказалась в верхней части окна, а не в нижней.
